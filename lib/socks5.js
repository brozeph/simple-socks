				...
				.tap(function (args) {
					if (args.cmd === RFC_1928_COMMANDS.CONNECT) {
						...
						destination.on('connect', function () {
							...
							self.server.emit(EVENTS.PROXY_CONNECT, args.dst.addr, args.dst.port);
						});
		...
		function handshake (buffer) {
			binary
				...
				.tap(function (args) {
					...
					var
						methods = [].slice.call(args.methods).reduce(function (methods, method) {
							methods[method] = true;
							return methods;
						}, {}),
						responseBuffer = put()
					...
					// supporting NO AUTHENTICATION REQUIRED only for now
					if (typeof methods[0] === 'undefined' || !methods[0]) {
					...
					socket.write(responseBuffer, function () {
						...
						self.server.emit(EVENTS.HANDSHAKE, args);
