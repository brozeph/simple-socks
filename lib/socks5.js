...
module.exports = (function (self) {
	...
	// local state
	// ??? wonder if hash table would be better than array
	//     concurrent splicing might be a problem
	self.activeSessions = [];
	self.options = {};
	self.server = null;

	function Session (socket) {
		var _this = this;  // reference indexOf for self.activeSessions
		...
		function end (response, args) {
			...
			// ??? under Linux, it erred out because socket.once('end') never get called
			//     so replace the above try/catch with this:
			// socket.end(responseBuffer);
			// socket.destroy();
			// self.activeSessions.splice(self.activeSessions.indexOf(_this), 1);
