...
function SocksServer (options) {
	...
	if(self.options.interface) {
		var interfaces = os.networkInterfaces();
    		if (!interfaces[interface]) throw new Error('unknown interface name ' + interface);
        	var iface = interfaces[interface].filter(function (f) {
        		return f.family == 'IPv4';
    		});
    		if (!iface || !iface[0]) throw new Error('no ' + 'IPv4' + ' family in ' + interfaces[interface]);
