...
function SocksServer (options) {
	...
	if(self.options.interface) {
		var interfaces = os.networkInterfaces();
    		if (!interfaces[self.options.interface]) throw new Error('unknown interface name ' + self.options.interface);
        	var iface = interfaces[self.options.interface].filter(function (f) {
        		return f.family == 'IPv4';
    		});
    		if (!iface || !iface[0]) throw new Error('no ' + 'IPv4' + ' family in ' + interfaces[self.options.interface]);
		self.ip = iface[0].address;
	}
			...
			binary
				...
				.tap(function (args) {
					if (args.cmd === RFC_1928_COMMANDS.CONNECT) {
						// if bind on interface
						if(self.options.interface) {
							var destination = new net.Socket({handle: net._createServerHandle(self.ip)});
							var r = current.setsockopt(destination._handle.fd, SOL_SOCKET, SO_BINDTODEVICE, self.options.interface, 6);
    							if (r === -1) throw new Error("getsockopt(SO_BINDTODEVICE) error");
						} else {
