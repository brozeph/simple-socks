...
const {
    EVENTS, RFC_1928_ATYP, RFC_1928_COMMANDS, RFC_1928_METHODS,
    RFC_1928_REPLIES, RFC_1928_VERSION, RFC_1929_REPLIES,
    RFC_1929_VERSION
} = require('./constants');
                    ...
                    if (args.cmd === RFC_1928_COMMANDS.CONNECT) {
                        const destination = net.createConnection(
                            ...
                            function () {
                                ...
                                let responseBuffer = new Buffer(args.requestBuffer.length);
        ...
        function handshake(buffer) {
            binary
                ...
                .tap(function (args) {
                    ...
                    let acceptedMethods = [].slice.call(args.methods).reduce(function (methods, method) {
                            methods[method] = true;
                            return methods;
                        }, {}),
